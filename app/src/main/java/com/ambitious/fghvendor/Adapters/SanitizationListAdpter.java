...
import com.ambitious.fghvendor.Activities.SanitizationProfileActivity;
import com.ambitious.fghvendor.Model.Sanitization;
...
public class SanitizationListAdpter extends RecyclerView.Adapter<SanitizationListAdpter.MyViewHolder> {

    private Context context;
    private ArrayList<Sanitization> sanitizations;

    public SanitizationListAdpter(Context context, ArrayList<Sanitization> sanitizations) {
        this.context = context;
        this.sanitizations = sanitizations;
        Collections.reverse(this.sanitizations);
    }

    @NonNull
    @Override
    public SanitizationListAdpter.MyViewHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        View view = LayoutInflater.from(context).inflate(R.layout.item_blood, parent, false);

        return new SanitizationListAdpter.MyViewHolder(view);
    }

    @Override
    public void onBindViewHolder(@NonNull SanitizationListAdpter.MyViewHolder holder, int position) {
        Sanitization donor = sanitizations.get(position);
        ...
        public MyViewHolder(@NonNull View itemView) {
            ...
            itemView.setOnClickListener(new View.OnClickListener() {
                ...
                public void onClick(View v) {
                    ...
                    context.startActivity(new Intent(context, SanitizationProfileActivity.class)
                            .putExtra("head", tv_Name.getText().toString())
                            .putExtra("obj", sanitizations.get(getAdapterPosition()).getObj()));
    ...
    public void updateList(ArrayList<Sanitization> list) {
        sanitizations = list;
