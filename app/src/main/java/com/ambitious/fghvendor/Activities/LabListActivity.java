...
import com.ambitious.fghvendor.Adapters.LabListAdapter;
...
public class LabListActivity extends AppCompatActivity implements View.OnClickListener {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        et_Search.addTextChangedListener(new TextWatcher() {
            ...
            public void afterTextChanged(Editable s) {
                ...
                if (labArrayList.size() > 0) {
                    filter(s.toString());
                } else {
                    CustomSnakbar.showDarkSnakabar(mContext, iv_Bck, "Labs is not available.");
                }
    ...
    private void filter(String text) {
        ...
        if (temp.size() > 0) {
            rv_Labs.setVisibility(View.VISIBLE);
            tv_Notavailable.setVisibility(View.GONE);
            adapter.updateList(temp);
        } else {
            rv_Labs.setVisibility(View.GONE);
                    ...
                    if (response.isSuccessful()) {
                        ...
                        if (status.equalsIgnoreCase("1")) {
                            ...
                            for (int i = 0; i < array.length(); i++) {
                                ...
                                lab.setRating(rating);
                                lab.setImage(img);
                                lab.setAvailable(available);
                                lab.setObj(result.toString());
                            ...
                            if (labArrayList.size() > 0) {
                                adapter = new LabListAdapter(mContext, labArrayList);
                                LinearLayoutManager manager = new LinearLayoutManager(mContext);
                                manager.setOrientation(RecyclerView.VERTICAL);
                                rv_Labs.setLayoutManager(manager);
                                rv_Labs.setAdapter(adapter);
                            } else {
    ...
    private void finds() {
        ...
        rv_Labs = findViewById(R.id.rv_Labs);
    ...
    public void onClick(View v) {
        ...
        switch (v.getId()) {
            ...
            case R.id.ll_Login:
                startActivity(new Intent(mContext, LabLoginActivity.class));
                Animatoo.animateCard(mContext);
                break;
        }
    }
